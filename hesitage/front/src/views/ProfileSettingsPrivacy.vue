        ...
        <main class="settings-main">
          <div class="privacy-grid">
            ...
            <section class="privacy-col split">
              <div class="privacy-title">活动动态</div>

              <!-- ✅ 公开我的信息（第一项） -->
              <label class="radio-row">
                <input class="radio" type="checkbox" v-model="privacy.showInfo" :disabled="privacyLocked" />
                <span class="radio-text">公开我的信息</span>
              </label>

              <!-- 公开我的收藏（第二项） -->
              <label class="radio-row">
                <input class="radio" type="checkbox" v-model="privacy.showFavorites" :disabled="privacyLocked" />
                <span class="radio-text">公开我的收藏</span>
              </label>
            </section>

            <!-- 位置信息 -->
            <section class="privacy-col split">
              <div class="privacy-title">位置信息</div>

              <label class="radio-row">
                <input class="radio" type="checkbox" v-model="privacy.showLocation" :disabled="privacyLocked" />
                <span class="radio-text">在地图上显示我的大致位置</span>
              </label>

              <div class="privacy-desc">
                此设置不会暴露您的精准位置仅用于显示您所在区域的文化活动
              </div>
            </section>
          </div>
...
const LS_PRIVACY_INFO = 'privacy_show_info'         // 公开我的信息（新）
const LS_PRIVACY_FAV = 'privacy_show_favorites'     // 1/0
const LS_PRIVACY_LOC = 'privacy_show_location'      // 1/0
// 兼容旧key（你之前叫“足迹”）
const LEGACY_LS_PRIVACY_FOOT = 'privacy_show_footprints'
...
const privacyLocked = computed(() => privacy.profileVisibility === 'private')

watch(
  () => privacy.profileVisibility,
  (v) => {
    if (v === 'private') {
      privacy.showInfo = false
      privacy.showFavorites = false
      privacy.showLocation = false
    }
  }
)

function loadPrivacyFromLS() {
  const vis = (localStorage.getItem(LS_PRIVACY_VIS) as ProfileVisibility | null) || 'public'
  privacy.profileVisibility = vis === 'private' ? 'private' : 'public'

  const infoRaw = localStorage.getItem(LS_PRIVACY_INFO)
  const legacyFootRaw = localStorage.getItem(LEGACY_LS_PRIVACY_FOOT)
  if (infoRaw !== null) {
    privacy.showInfo = infoRaw === '1'
  } else if (legacyFootRaw !== null) {
    privacy.showInfo = legacyFootRaw === '1'
  } else {
    privacy.showInfo = true
  }

  privacy.showFavorites = localStorage.getItem(LS_PRIVACY_FAV)
    ? localStorage.getItem(LS_PRIVACY_FAV) === '1'
    : true

  privacy.showLocation = localStorage.getItem(LS_PRIVACY_LOC)
    ? localStorage.getItem(LS_PRIVACY_LOC) === '1'
    : true
...
function onSavePrivacy() {
  localStorage.setItem(LS_PRIVACY_VIS, privacy.profileVisibility)
  localStorage.setItem(LS_PRIVACY_INFO, privacy.showInfo ? '1' : '0')
  localStorage.setItem(LS_PRIVACY_FAV, privacy.showFavorites ? '1' : '0')
  localStorage.setItem(LS_PRIVACY_LOC, privacy.showLocation ? '1' : '0')

  localStorage.setItem(LEGACY_LS_PRIVACY_FOOT, privacy.showInfo ? '1' : '0')
